...
DECLARE
    ...
    search_strings string_list := string_list('POR_TSS_OPE',
	'POR_TST_OPE',
	'VAL_ATN_RNE',
	'VAL_TPO_CBO_OPE');
    
    -- Texto objetivo donde buscaremos
    target_text VARCHAR2(32767) := 'create or replace PROCEDURE SP_CNFG_GBR_TBL_FNL (
...
BEGIN
        ...
        FROM
            ...
            JOIN CNFGS_IN_CONTARCSTGLOBALSAN ON CNFGS_IN_PARTICIPANTS.UTILIZATIONIDSERVICING = CNFGS_IN_CONTARCSTGLOBALSAN.UTILIZATIONIDSERVICING
        ...
        SELECT
            ...
            0 AS NUM_CCB_INT,
            FLOW_DATA.IMP_SDO_INT AS IMP_SDO_INT,
            CASE
    ...
    FOR i IN 1..search_strings.COUNT LOOP
        FOR line IN (SELECT REGEXP_SUBSTR(target_text, '[^' || CHR(10) || ']+', 1, LEVEL) AS line
            ...
            IF INSTR(line.line, search_strings(i)) > 0 THEN
                ...
                DBMS_OUTPUT.PUT_LINE('Cadena encontrada: ' || search_strings(i) || ' Línea ' || line_number || ': ' || line.line);
                found := TRUE;
            END IF;
